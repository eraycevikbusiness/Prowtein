  ...
  return (
    <div className="max-w-[940px] mx-auto px-7 py-7 flex flex-col gap-5">
      ...
      <div className="rounded-2xl bg-white border border-border p-6 flex flex-col sm:flex-row items-center gap-7">
        <Ring value={totals.protein} goal={goals.protein} label="Protein" unit="g" />
        <div className="flex-1 min-w-0 w-full">
          <div className="flex items-baseline justify-between mb-1">
            <span className="text-[12.5px] text-ink-3">Calories</span>
            <span className="text-[12px] text-ink-3">of {formatKcal(goals.calories)} kcal</span>
          </div>
          <div className="flex items-baseline gap-2 mb-3">
            <span className="font-serif text-[40px] leading-none">{formatKcal(totals.kcal)}</span>
            <span className="text-[14px] text-ink-3">kcal</span>
          </div>
          ...
          <div className="mt-2.5 flex flex-wrap items-center gap-4 text-[12px] text-ink-2">
            <span className="inline-flex items-center gap-1.5">
              ...
              Protein <span className="text-ink-3">{round1(totals.protein)} g</span>
            </span>
            <span className="inline-flex items-center gap-1.5">
              <span className="macro-dot" style={{ background: MACRO.carbs }} />
              Carbs <span className="text-ink-3">{round1(totals.carbs)} g</span>
            </span>
            <span className="inline-flex items-center gap-1.5">
              <span className="macro-dot" style={{ background: MACRO.fat }} />
              Fat <span className="text-ink-3">{round1(totals.fat)} g</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-baseline justify-between mb-3">
          <h3 className="text-[15px] font-semibold tracking-tight">Today's meals</h3>
        ...
        {entries.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[var(--border-2)] bg-white/40 dot-grid px-6 py-12 text-center">
            ...
            <p className="text-[15px] text-ink-2">Nothing logged yet.</p>
            <p className="text-[13px] text-ink-3 mt-1">
              Press <span className="chip">⌘K</span>, type three letters, hit return.
            </p>
