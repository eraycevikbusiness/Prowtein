...
// Darkened palette tints so the small uppercase labels read on cream.
const KIND_COLOR: Record<ChangeKind, string> = {
  new: "#C8442F", // accent-deep
  added: "#6E8B52", // sage, darkened
  changed: "#BD7A36", // apricot, darkened
  fixed: "#735B78", // plum, darkened
  notes: "#8A8580", // ink-3
};
      ...
      <div className="relative max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <div className="mt-16 relative pl-7 sm:pl-12">
          ...
          <div className="space-y-16">
            {RELEASES.map((r, i) => {
              const isUnreleased = r.version === "unreleased";
              return (
                <motion.article
                  key={r.version}
                ...
                >
                    ...
                    <span
                      className={
                        ...
                        (r.status === "latest"
                          ? "bg-[var(--accent)]"
                          : isUnreleased
                            ? "bg-white border-2 border-dashed border-[var(--border-2)]"
                            : "bg-[var(--ink-4)]")
                      }
                    />
                  </span>
                  ...
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-2">
                    <h2 className="font-serif text-[28px] leading-none text-[var(--ink)]">
                      {isUnreleased ? c.status.unreleased : `v${r.version}`}
                    </h2>
                    {r.status === "latest" && (
                    ...
                    {r.status === "pre" && (
                      <span className="inline-flex items-center h-[20px] px-2 rounded-full bg-[var(--bg-2)] text-[var(--ink-3)] text-[10.5px] font-medium uppercase tracking-wider">
                        {c.status.pre}
                      </span>
                    )}
                    <span className="text-[13px] text-[var(--ink-3)] ml-auto">{r.date}</span>
                  </div>
                  <p className="mt-2 text-[15.5px] text-[var(--ink-2)] font-medium">{r.title}</p>
        ...
        <div className="mt-20 pt-8 border-t border-[var(--border)] flex items-center gap-2.5 text-[13.5px] text-[var(--ink-3)]">
          ...
          <span>Built incrementally, vibe-coded with Claude.</span>
        </div>
      </div>
    </main>
  );
